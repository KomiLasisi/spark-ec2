...
# Pre-packaged spark version:
  ...
  case "$SPARK_VERSION" in
    ...
    1.0.0)
      if [[ "$HADOOP_MAJOR_VERSION" == "1" ]]; then
        wget http://people.apache.org/~tdas/spark-1.0.0-rc11/spark-1.0.0-bin-hadoop1.tgz
      else
        wget http://people.apache.org/~tdas/spark-1.0.0-rc11/spark-1.0.0-bin-cdh4.tgz
      fi
      ;;
    *)
